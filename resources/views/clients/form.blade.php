    ...
    <div class="container-fluid">
        ...
        <div class="form-layout-wrapper">
            <div class="card-style mb-30">
                ...
                @if(isset($item))
                <form method="POST" action="{{route('updateClient',['id'=>$item->id])}}" id="clientsForm"
                    enctype="multipart/form-data">
                    @else
                    <form method="POST" action="{{route('storeClient')}}" id="clientsForm"
                        enctype="multipart/form-data">
                        @endif
                        <div class="row">
                            @csrf
                            ...
                            <div class="col-md-4 col-sm-12 col-12">
                                <div class="input-style-1">
                                    ...
                                    @if(isset($item))
                                    <input type="text" name="email" id="email" class="bg-transparent"
                                        placeholder="Email" value="@isset($item->email){{$item->email}}@endisset"
                                        readonly>
                                    @else
                                    <input type="text" name="email" id="email" class="bg-transparent"
                            ...
                            <div class="col-md-6 col-sm-12 col-12">
                                <div class="input-style-1">
                                    ...
                                    <input type="text" name="pan_card_no" id="pan_card_no" class="bg-transparent" onkeyup="checkValidation()" placeholder="PAN Card No Ex: ABCDE1234F" value="@isset($item->pan_card_no) {{$item->pan_card_no}}@endisset">
                                </div>
                            </div>
                            <div class="col-md-6 col-sm-12 col-12">
                                <div class="input-style-1">
                                    <label for="aadhaar_card_no">Aadhaar Card No</label>
                                    <input type="text" name="aadhaar_card_no" id="aadhaar_card_no"
                                        class="bg-transparent" onkeyup="checkValidation()" placeholder="Aadhaar Card No Ex: 1234 5678 9012"
...
const checkValidation = debounce(()=> {
    if($('#pan_card_no').val()){
        $("#pan_card_no").rules("add", { pndcheck: true, messages: { pndcheck: "Invalid Pan Card Number"}});
    }else if($('#aadhaar_card_no').val()){
        $("#aadhaar_card_no").rules("add", { aadharcheck: true, messages: { aadharcheck: "Invalid Aadhaar Card Number"}});
    }else{
